...
from app.api.deps import require_workspace_admin
...
def _history(db: Session, organization_id: int) -> tuple[list[Decimal], list[Decimal]]:
    markups: list[Decimal] = []
    variances: list[Decimal] = []
    projects = db.query(Project).filter(
        Project.organization_id == organization_id,
        Project.quote_id.is_not(None),
    ).all()
    for project in projects:
        quote = db.query(Quote).filter(
            Quote.id == project.quote_id,
            Quote.organization_id == organization_id,
        ).one_or_none()
        if not quote:
            continue
        real_cost = Decimal(
            db.query(func.coalesce(func.sum(ProjectCost.total_cost), 0)).filter(
                ProjectCost.project_id == project.id,
                ProjectCost.organization_id == organization_id,
            ).scalar()
        )
        if real_cost <= 0 or Decimal(quote.total or 0) <= 0:
            continue
...
def build_quote_recommendation(
    *, db: Session, organization_id: int, base_cost: Decimal, requested_margin: Decimal
) -> dict[str, Decimal | int | str]:
    markups, variances = _history(db, organization_id)
...
@router.post("/recommend", dependencies=[Depends(require_workspace_admin)])
def recommend_quote(
    payload: QuoteRecommendationRequest,
    current_user: User = Depends(require_workspace_admin),
    db: Session = Depends(get_db),
):
    base_cost = payload.material_cost + payload.hardware_cost + payload.labor_cost + payload.finishing_cost
    ...
    return {
        ...
        **{key: format(value, ".2f") if isinstance(value, Decimal) else value for key, value in result.items()},
    }

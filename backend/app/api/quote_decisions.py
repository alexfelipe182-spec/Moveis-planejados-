...
def decide_quote(
    ...
    label = "Aprovou" if payload.status == "approved" else "Rejeitou"
    db.add(_activity(current_user, payload.status, "quote", item.id, f"{label} quote #{item.id}"))
    db.commit()
    db.refresh(item)
    engine.emit(
...
def record_quote_share(
    ...
    db.add(
        _activity(
            ...
            "quote",
            item.id,
            f"Registrou envio da proposta do quote #{item.id} ao cliente",
        )
    )
    db.commit()
    db.refresh(item)
...
def update_quote_commercial_status(
    ...
    db.add(
        _activity(
            ...
            "quote",
            item.id,
            f"Registrou que o cliente {label} a proposta do quote #{item.id}",
        )
    )
    if payload.status == "accepted":
        existing_project = (
            tenant_query(db, Project, current_user)
            .filter(Project.quote_id == item.id)
            .one_or_none()
        )
        if existing_project:
            raise HTTPException(

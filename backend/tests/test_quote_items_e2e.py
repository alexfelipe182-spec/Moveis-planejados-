...
def test_quote_items_crud_and_subtotal():
    ...
    registered = client.post("/api/v1/auth/register", json={"name": "Quote Items Admin", "email": email, "password": password})
    ...
    customer = client.post("/api/v1/customers", json={"name": "Cliente Itens E2E"}, headers=headers)
    ...
    item = client.post(
        ...
        json={"name": "Armário", "quantity": "2", "unit_price": "1250.00", "width": "2.40", "height": "2.10"},
        headers=headers,
    )
    ...
    deleted = client.delete(f"/api/v1/quotes/{quote_id}/items/{item.json()['id']}", headers=headers)

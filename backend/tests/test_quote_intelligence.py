...
def test_recommendation_raises_margin_when_history_shows_cost_overrun():
    result = recommend_from_history(
        base_cost=Decimal("10000.00"),
        requested_margin=Decimal("20.00"),
        historical_margins=[Decimal("18.00"), Decimal("20.00"), Decimal("22.00")],
        historical_cost_variances=[Decimal("10.00"), Decimal("12.00"), Decimal("8.00")],
    )

    assert result["sample_size"] == 3
    assert result["average_real_margin_percent"] == Decimal("20.00")
